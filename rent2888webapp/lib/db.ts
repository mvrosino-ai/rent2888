import { Pool } from "pg";

export type UserRole = "ADMIN" | "OWNER";

export interface DbUser {
  id: string;
  email: string;
  password_hash: string;
  role: UserRole;
  propietario_name: string | null;
  active: boolean;
  created_at: string;
}

let pool: Pool | null = null;

function db(): Pool {
  if (!pool) {
    const url = process.env.DATABASE_URL;
    if (!url) throw new Error("DATABASE_URL no está configurada");
    pool = new Pool({ connectionString: url, max: 3 });
  }
  return pool;
}

export async function findUserByEmail(email: string): Promise<DbUser | null> {
  const { rows } = await db().query<DbUser>(
    `SELECT id, email, password_hash, role, propietario_name, active, created_at
     FROM users WHERE lower(email) = lower($1) LIMIT 1`,
    [email]
  );
  return rows[0] ?? null;
}

export async function listUsers(): Promise<DbUser[]> {
  const { rows } = await db().query<DbUser>(
    `SELECT id, email, password_hash, role, propietario_name, active, created_at
     FROM users ORDER BY created_at`
  );
  return rows;
}

export async function createUser(u: {
  email: string;
  passwordHash: string;
  role: UserRole;
  propietarioName: string | null;
}): Promise<void> {
  await db().query(
    `INSERT INTO users (email, password_hash, role, propietario_name)
     VALUES ($1, $2, $3, $4)`,
    [u.email, u.passwordHash, u.role, u.propietarioName]
  );
}

export async function setUserActive(id: string, active: boolean): Promise<void> {
  await db().query(`UPDATE users SET active = $2 WHERE id = $1`, [id, active]);
}

export async function updateUserPassword(id: string, passwordHash: string): Promise<void> {
  await db().query(`UPDATE users SET password_hash = $2 WHERE id = $1`, [id, passwordHash]);
}

export async function updateUserPropietario(
  id: string,
  propietarioName: string | null
): Promise<void> {
  await db().query(`UPDATE users SET propietario_name = $2 WHERE id = $1`, [id, propietarioName]);
}

export async function deleteUser(id: string): Promise<void> {
  await db().query(`DELETE FROM users WHERE id = $1`, [id]);
}

export async function getCommissionPct(): Promise<number> {
  try {
    const { rows } = await db().query(`SELECT commission_pct FROM settings LIMIT 1`);
    if (rows[0]) return Number(rows[0].commission_pct);
  } catch {
    // sin DB o sin tabla settings: cae al env var
  }
  return Number(process.env.COMMISSION_PCT ?? 0.2);
}

export async function setCommissionPct(pct: number): Promise<void> {
  await db().query(
    `INSERT INTO settings (id, commission_pct) VALUES (TRUE, $1)
     ON CONFLICT (id) DO UPDATE SET commission_pct = $1`,
    [pct]
  );
}
